...
NUITKA_WIN_FLAGS := --windows-service --windows-service-name=TLSCertMonitor --windows-service-display-name="TLS Certificate Monitor" --windows-service-description="Monitor TLS/SSL certificates for expiration and security issues" --windows-service-install="install" --windows-service-uninstall="uninstall" --windows-service-cmdline="--config C:\\ProgramData\\tls-cert-monitor\\config.yaml"
